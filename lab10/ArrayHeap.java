...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        int minindexwithleftchild = min(leftchildindex, index);
        int minindexwithrightchild = min(rightchildindex, index);
        if(minindexwithleftchild != index && minindexwithrightchild != index) {
            int minleftwithright = min(minindexwithrightchild, minindexwithleftchild);
            if(minleftwithright == rightchildindex) {
        ...
        } else if(minindexwithleftchild == index && minindexwithrightchild != index) {
            swap(index, rightchildindex);
            sink(rightchildindex);
        } else if(minindexwithleftchild != index) {
            swap(index, leftchildindex);
            sink(leftchildindex);
        } else {
            return;
    ...
    public T removeMin() {
        ...
        if(size == 0) {
            ...
            swap(1, size);
            T returnitem = contents[size].myItem;
            contents[size] = null;
            size--;
            sink(1);
            return returnitem;
    ...
    private Node FindNode(int index, T item) {
        if(index > size) {
            return null;
        } else {
            if(contents[index].myItem.equals(item)) {
                ...
                Node node1 = FindNode(index * 2, item);
                Node node2 = FindNode(index * 2 + 1, item);
                if(node1 != null) {
                    return node1;
                } else {
                    return node2;
